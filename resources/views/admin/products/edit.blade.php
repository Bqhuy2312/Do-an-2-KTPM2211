...
@section('content')
    ...
    <form action="{{ route('products.update', $product->id) }}" method="POST" enctype="multipart/form-data">
        ...
        <div>
            <label>
                <input type="checkbox" name="best_seller" value="1" {{ old('best_seller', $product->best_seller) ? 'checked' : '' }}>
                Bán chạy
            </label>
        </div>
            ...
            @foreach($product->variants as $variant)
                <div class="variant-item">
                    ...
                    <div>
                        ...
                        <input type="number" name="quantities[]" min="0" value="{{ old('quantities.' . $loop->index, $variant->quantity) }}" required>
                    </div>
